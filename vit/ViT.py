...
def train_ViT(model, optimizer, train_loader, valid_loader,
    n_epochs=100, patience=25, criterion=nn.CrossEntropyLoss(),
    train_losses=[], valid_losses=[], prefix="CNN", feature_extractor=None):
    ...
    def _train():
        ...
        for batch in tqdm(train_loader):
            ...
            inputs = feature_extractor(images=imgs, return_tensors="pt", do_rescale=False)
...
def load_model_ViT(model_path, loss_path = None):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
def try_load_model_ViT(exp_name):
